...
def test_index_GET():
    ...
    assert "album_id" in sample_album_jsobject and isinstance(
        sample_album_jsobject.get("album_id"), int
    )
    assert "title" in sample_album_jsobject and isinstance(
        sample_album_jsobject.get("title"), str
    )
    assert "number_of_discs" in sample_album_jsobject and isinstance(
        sample_album_jsobject.get("number_of_discs"), int
    )
    assert "number_of_tracks" in sample_album_jsobject
    assert isinstance(sample_album_jsobject.get("number_of_tracks"), int)
    assert "release_date" in sample_album_jsobject and isinstance(
        sample_album_jsobject.get("release_date"), str
    )
...
def test_index_POST():
    ...
    request = request_factory.post(
        "/albums/", data=new_album_dict, content_type="application/json"
    )
    response = index(request)
    assert isinstance(response, JsonResponse)
    json_content = json.loads(response.content)
...
def test_single_album_GET():
    ...
    # object yields an IntegrityError that claims a pre-existing primary
    # key column value was used. This when no primary key column value
    # was set. (This bug is likely in pytest-django, not psycopg2.) This
...
def test_single_album_GET_error_nonexistent_album_id():
    ...
    assert (
        ...
        and json_content["message"] == f"no album with album_id={album_id}"
    )
...
def test_single_album_PATCH():
    ...
    request = request_factory.patch(
        f"/albums/{album_id}", data=album_patch_dict, content_type="application/json"
    )
    response = single_album(request, album_id)
    json_content = json.loads(response.content)
...
def test_single_album_songs_GET():
    ...
    assert "length_minutes" in track_dict and isinstance(
        track_dict["length_minutes"], int
    )
    assert "length_seconds" in track_dict and isinstance(
        track_dict["length_seconds"], int
    )
    assert "song_lyrics_id" in track_dict and isinstance(
        track_dict["song_lyrics_id"], int
    )
...
def test_single_album_single_song_GET():
    ...
    assert "length_minutes" in json_content and isinstance(
        json_content["length_minutes"], int
    )
    assert "length_seconds" in json_content and isinstance(
        json_content["length_seconds"], int
    )
...
def test_single_album_genres_GET():
    ...
    assert "genre_id" in sample_genre_dict and isinstance(
        sample_genre_dict["genre_id"], int
    )
    assert "genre_name" in sample_genre_dict and isinstance(
        sample_genre_dict["genre_name"], str
    )
...
def test_single_album_genres_POST():
    ...
    assert (
        "genre_name" in json_content and json_content["genre_name"] == genre.genre_name
    )
    bridge_rows = AlbumGenreBridge.objects.filter(album_id=album_id)
...
def test_single_album_artists_GET():
    ...
    assert "artist_id" in sample_artist_dict and isinstance(
        sample_artist_dict["artist_id"], int
    )
    assert "first_name" in sample_artist_dict and isinstance(
        sample_artist_dict["first_name"], str
    )
    assert "last_name" in sample_artist_dict and isinstance(
        sample_artist_dict["last_name"], str
    )
    assert "gender" in sample_artist_dict and sample_artist_dict["gender"] in (
        "male",
        "female",
        "nonbinary",
    )
    assert "birth_date" in sample_artist_dict and matches_date_isoformat(
        sample_artist_dict["birth_date"]
    )
...
def test_single_album_artists_POST():
    ...
    assert (
        "first_name" in json_content and json_content["first_name"] == artist.first_name
    )
    ...
    assert (
        "birth_date" in json_content
        and json_content["birth_date"] == artist.birth_date.isoformat()
    )
    bridge_rows = ArtistAlbumBridge.objects.filter(album_id=album_id)
