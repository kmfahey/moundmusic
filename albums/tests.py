...
def test_single_album_DELETE():
    ...
    assert json_content['message'] == f'album record with album_id={album_id} deleted'
    try:
...
def test_single_album_single_song_DELETE():
    ...
    json_content = json_loads(response.content)
    assert 'message' in json_content
    assert json_content['message'] == f'song record with album_id={album_id} and song_id={song_id} deleted'
    try:
...
def test_single_album_single_genre_DELETE():
    ...
    json_content = json_loads(response.content)
    assert 'message' in json_content
    assert json_content['message'] == f'association between album record with album_id={album_id} and genre record with genre_id={genre_id} deleted'
    try:
