...
class Table_Seeder(object):
    __slots__ = (
        ...
        "genre_ids",
        "song_ids",
        "total_tracks_on_albums",
        "song_ids_to_incidences",
        "song_ids_to_song_lyrics_ids",
        "song_ids_to_song_lyrics_ids",
        "song_ids",
        "album_ids_to_artist_ids",
        "album_ids_to_params",
        "album_ids_to_tracklists",
        "user_ids",
        "user_ids_to_seller_ids",
        "user_ids_to_buyer_ids",
    )

    Lyrics_Wordcount = 500

    Album_Params = collections.namedtuple(
        "Album_Params", ("number_of_discs", "number_of_tracks")
    )

    Album_Song_Bridge_Row = collections.namedtuple(
        "Album_Song_Bridge_Row", ("album_id", "disc_number", "track_number", "song_id")
    )
    ...
    def seed_album_table(self):
        ...
        for _ in range(100):
            ...
            number_of_tracks = (
                ...
                else random.randint(16, 40)
            )
            album_params_list.append(
                self.Album_Params(number_of_discs, number_of_tracks)
            )
            release_date = self.faker_obj.date_between_dates(
                datetime.date(1990, 1, 1), datetime.date(2022, 12, 31)
            )
            self.cursor.execute(
                "INSERT INTO album (title, number_of_discs, number_of_tracks, release_date)"
                "VALUES (%s, %s, %s, %s);",
                (title, number_of_discs, number_of_tracks, release_date),
            )
    ...
    def seed_artist_table(self):
        ...
        for _ in range(25):
            ...
            birth_date = self.faker_obj.date_between_dates(
                datetime.date(1925, 1, 1), datetime.date(1989, 12, 31)
            )
            self.cursor.execute(
                "INSERT INTO artist (first_name, last_name, gender, birth_date)"
                "VALUES (%s, %s, %s, %s);",
    ...
    def seed_genre_table(self):
        ...
        for _ in range(10):
            ...
            if genre_word_count == 1:
                ...
                genre = title_case(
                    " ".join((self.faker_obj.word("noun"), self.faker_obj.word("noun")))
                )
    ...
    def seed_song_table(self):
        ...
        while tracks_so_far < self.total_tracks_on_albums:
            ...
            self.cursor.execute(
                "INSERT INTO song (title, length_minutes, length_seconds) "
                "VALUES (%s, %s, %s);",
                (title, length_minutes, length_seconds),
            )
    ...
    def seed_song_lyrics_table(self):
        ...
        for song_id in sorted(self.song_ids_to_incidences):
            ...
            self.cursor.execute(
                "INSERT INTO song_lyrics (lyrics, song_id) VALUES (%s, %s);",
                (lyrics, song_id),
            )
        self.cursor.execute("COMMIT;")
    ...
    def update_song_table_with_song_lyrics_ids(self):
        for song_id, song_lyrics_id in self.song_ids_to_song_lyrics_ids.items():
            self.cursor.execute(
                "UPDATE song SET song_lyrics_id = %s WHERE song_id = %s;",
                (song_lyrics_id, song_id),
            )
        self.cursor.execute("COMMIT;")
    ...
    def _seed_x_genre_bridge_table(self, bridge_to):
        ...
        insert_sql = (
            f"INSERT INTO {table_name} ({bridge_to}_id, genre_id) VALUES (%s, %s);"
        )
    ...
    def seed_artist_album_bridge_table(self):
        ...
        for album_id in self.album_ids:
            ...
            for _ in range(number_of_artists):
                ...
                self.cursor.execute(
                    "SELECT artist_id, album_id FROM artist_album_bridge WHERE album_id = %s AND artist_id = %s;",
                    (album_id, artist_id),
                )
                if self.cursor.fetchall():
                    continue
                self.cursor.execute(
                    "INSERT INTO artist_album_bridge (album_id, artist_id) VALUES (%s, %s);",
                    (album_id, artist_id),
                )
    ...
    def seed_album_song_bridge_table(self):
        ...
        for album_id, album_params in self.album_ids_to_params.items():
            ...
            if album_params.number_of_discs == 1:
                album_id_to_disc_numbers_track_numbers[album_id] = list(
                    (1, track_number)
                    for track_number in range(1, album_params.number_of_tracks + 1)
                )
            else:
                ...
                while track_number <= album_params.number_of_tracks:
                    ...
                    track_number += 1
                album_id_to_disc_numbers_track_numbers[
                    album_id
                ] = disc_numbers_track_numbers
                assert len(disc_numbers_track_numbers) == album_params.number_of_tracks
        song_ids = list(song_ids_to_incidences)
        for (
            album_id,
            disc_numbers_track_numbers,
        ) in album_id_to_disc_numbers_track_numbers.items():
            tracklist = list()
            for disc_number, track_number in sorted(disc_numbers_track_numbers):
                ...
                album_song_bridge_row = self.Album_Song_Bridge_Row(
                    album_id, 1, track_number, song_id
                )
                insert_values_list.append(album_song_bridge_row)
                tracklist.append(
                    (
                        album_song_bridge_row.disc_number,
                        album_song_bridge_row.track_number,
                        song_id,
                    )
                )
        ...
        for album_song_bridge_row in insert_values_list:
            self.cursor.execute(
                "SELECT album_id, song_id FROM album_song_bridge WHERE album_id = %s AND song_id = %s;",
                (album_song_bridge_row.album_id, album_song_bridge_row.song_id),
            )
            if self.cursor.fetchall():
                continue
            self.cursor.execute(
                "INSERT INTO album_song_bridge (album_id, disc_number, track_number, song_id)"
                "VALUES (%s, %s, %s, %s);",
                album_song_bridge_row,
            )
        self.cursor.execute("COMMIT;")
        ...
        for album_id, tracklist in self.album_ids_to_tracklists.items():
            ...
            for artist_id in self.album_ids_to_artist_ids[album_id]:
                for _, _, song_id in tracklist:
                    artist_id = random.choice(self.artist_ids)
                    self.cursor.execute(
                        "SELECT artist_id, song_id FROM artist_song_bridge WHERE song_id = %s AND artist_id = %s;",
                        (song_id, artist_id),
                    )
                    if self.cursor.fetchall():
                        continue
                    self.cursor.execute(
                        "INSERT INTO artist_song_bridge (song_id, artist_id) "
                        "VALUES (%s, %s);",
                        (song_id, artist_id),
                    )
    ...
    def seed_user_table(self):
        ...
        for _ in range(50):
            ...
            date_joined = self.faker_obj.date_between_dates(
                datetime.date.today() - datetime.timedelta(days=(365.24 * 3)),
                datetime.date.today(),
            )
            self.cursor.execute(
                "INSERT INTO user_ (user_name, first_name, last_name, gender, date_joined) "
                "VALUES (%s, %s, %s, %s, %s);",
                (user_name, first_name, last_name, gender, date_joined),
            )
        self.cursor.execute("COMMIT;")
    ...
    def seed_user_password_table(self):
        ...
        for user_id in self.user_ids:
            ...
            self.cursor.execute(
                "INSERT INTO user_password (encrypted_password, user_id) VALUES (%s, %s);",
                (encrypted_password, user_id),
            )
        self.cursor.execute("COMMIT;")

    def _seed_buyer_seller_account_table(
        self, table_name, id_col_name, display_col_name, save_dict_attr_name
    ):
        ...
        for user_id in self.user_ids:
            if random.randint(1, 10) in (1, 2, 3, 4, 5, 6):
                display_col_value = title_case(
                    " ".join(self.faker_obj.words(random.randint(2, 6)))
                )
                three_years_ago_date = datetime.date.today() - datetime.timedelta(
                    days=(365.24 * 3)
                )
                date_created = self.faker_obj.date_between_dates(
                    three_years_ago_date, datetime.date.today()
                )
                self.cursor.execute(
                    insert_sql, (display_col_value, date_created, user_id)
                )
        self.cursor.execute("COMMIT;")
    ...
    def seed_seller_account_table(self):
        self._seed_buyer_seller_account_table(
            "seller_account", "seller_id", "storefront_name", "user_ids_to_seller_ids"
        )

    def update_user_table_with_seller_ids(self):
        for user_id, seller_id in self.user_ids_to_seller_ids.items():
            self.cursor.execute(
                "UPDATE user_ SET seller_id = %s WHERE user_id = %s;",
                (seller_id, user_id),
            )
        self.cursor.execute("COMMIT;")
    ...
    def seed_buyer_account_table(self):
        self._seed_buyer_seller_account_table(
            "buyer_account", "buyer_id", "postboard_name", "user_ids_to_buyer_ids"
        )

    def update_user_table_with_buyer_ids(self):
        for user_id, buyer_id in self.user_ids_to_buyer_ids.items():
            self.cursor.execute(
                "UPDATE user_ SET buyer_id = %s WHERE user_id = %s;",
                (buyer_id, user_id),
            )
        self.cursor.execute("COMMIT;")

    def _seed_to_buy_or_sell_listing_table(
        self, table_name, id_col_name, price_col_name, saved_dict_attr_name
    ):
        delete_sql = f"DELETE FROM {table_name};"
        insert_sql = (
            f"INSERT INTO {table_name} ({price_col_name}, date_posted, album_id, {id_col_name}) "
            "VALUES (%s, %s, %s, %s);"
        )
        self.cursor.execute(delete_sql)
        three_months_ago_date = datetime.date.today() - datetime.timedelta(
            days=(365.24 / 12 * 3)
        )
        for id_datum in getattr(self, saved_dict_attr_name).values():
            for _ in range(random.randint(1, 5)):
                ...
                date_posted = self.faker_obj.date_between_dates(
                    three_months_ago_date, datetime.date.today()
                )
                self.cursor.execute(
                    insert_sql, (price, date_posted, album_id, id_datum)
                )
        self.cursor.execute("COMMIT;")

    def seed_to_buy_listing_table(self):
        self._seed_to_buy_or_sell_listing_table(
            "to_buy_listing", "buyer_id", "max_accepting_price", "user_ids_to_buyer_ids"
        )

    def seed_to_sell_listing_table(self):
        self._seed_to_buy_or_sell_listing_table(
            "to_sell_listing", "seller_id", "asking_price", "user_ids_to_seller_ids"
        )
