...
from .views import single_song_albums, single_song_single_album, single_song_lyrics, single_song_single_lyrics
...
def test_single_song_albums_GET():
    ...
    assert 'track_number' in sample_object and isinstance(sample_object['track_number'], int)
...
def test_single_song_song_lyrics_POST():
    ...
    request = request_factory.post(f"/songs/{song_id}/lyrics",
                                   data={'lyrics': lyrics},
...
def test_single_song_song_lyrics_POST_error_nonexistent_song_id():
    ...
    while True:
        ...
        if song_id not in song_ids:
            break
    song_lyrics_ids = [song_lyrics.song_lyrics_id for song_lyrics in SongLyrics.objects.filter()]
    song_lyrics_id = random.choice(song_lyrics_ids)
    new_song_lyrics_assoc_dict = {"song_lyrics_id": song_lyrics_id}
    request = request_factory.post(f"/songs/{song_id}/song_lyrics",
