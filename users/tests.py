...
from .views import (
    single_user_password_set_password,
    single_user_password_authenticate,
...
def test_single_user_password_set_password_POST():
    ...
    assert "password_id" in json_content and isinstance(
        json_content.get("password_id"), int
    )
    assert "encrypted_password" in json_content and isinstance(
        json_content.get("encrypted_password"), str
    )
...
def test_single_user_password_set_password_POST_error_nonexistent_user_id():
    ...
    json_content = json.loads(response.content)
    assert (
        "message" in json_content
        and json_content["message"] == f"no user with user_id={user_id}"
    )
...
def test_single_user_password_set_password_POST_error_extra_properties():
    ...
    request = request_factory.post(
        f"/users/{user_id}/password",
        data={"password": password, "foo": "bar"},
        content_type="application/json",
    )
    response = single_user_password_set_password(request, user_id)
    assert isinstance(response, JsonResponse)
    ...
    assert (
        "message" in json_content
        and json_content["message"] == "unexpected property in input: 'foo'"
    )
...
def test_single_user_password_authenticate_POST():
    ...
    user_password.save()
    request = request_factory.post(
        f"/users/{user_id}/password/authenticate",
        data={"password": password},
        content_type="application/json",
    )
...
def test_single_user_password_authenticate_POST_error_no_password_set():
    ...
    assert (
        "message" in json_content
        and json_content["message"]
        == f"user with user_id={user_id} has no password set"
    )
...
def test_single_user_any_buyer_account_GET():
    ...
    assert "postboard_name" in json_content and isinstance(
        json_content["postboard_name"], str
    )
    assert (
        "date_created" in json_content
        and isinstance(json_content["date_created"], str)
        and matches_date_isoformat(json_content["date_created"])
    )
...
def test_single_user_any_buyer_account_GET_error_user_has_no_buyer_account():
    ...
    assert (
        "message" in json_content
        and json_content["message"] == f"user with user_id={user_id} has no "
...
def test_single_user_any_buyer_account_POST():
    ...
    request = request_factory.post(
        f"/users/{user_id}/buyer_account",
        data=new_buyer_acct_args,
        content_type="application/json",
    )
    response = single_user_any_buyer_account(request, user_id)
    assert isinstance(response, JsonResponse)
    ...
    assert "buyer_id" in json_content and isinstance(json_content["buyer_id"], int)
    assert (
        "postboard_name" in json_content
        and json_content["postboard_name"] == new_buyer_acct_args["postboard_name"]
    )
    assert (
        "date_created" in json_content
        and isinstance(json_content["date_created"], str)
...
def test_single_user_any_buyer_account_POST_error_user_already_has_a_buyer_account():
    ...
    assert (
        "message" in json_content
        and json_content["message"] == f"user with user_id={user_id} already has a "
...
def test_single_user_single_buyer_account_GET_error_nonexistent_user_id():
    ...
    while True:
        ...
        if user_id not in user_ids:
            break
    buyer_ids = [
        buyer_account.buyer_id for buyer_account in BuyerAccount.objects.filter()
    ]
...
def test_single_user_single_buyer_account_GET_error_nonexistent_buyer_id():
    ...
    assert (
        "message" in json_content
        and json_content["message"] == f"no buyer account with buyer_id={buyer_id}"
    )
...
def test_single_user_single_buyer_account_any_listing_GET():
    ...
    assert "to_buy_listing_id" in listing_dict and isinstance(
        listing_dict["to_buy_listing_id"], int
    )
    assert (
        "max_accepting_price" in listing_dict
        and isinstance(listing_dict["max_accepting_price"], str)
        and matches_2plc_decimal_format(listing_dict["max_accepting_price"])
    )
    assert (
        "date_posted" in listing_dict
        and isinstance(listing_dict["date_posted"], str)
        and matches_date_isoformat(listing_dict["date_posted"])
    )
...
def test_single_user_single_buyer_account_any_listing_POST():
    ...
    new_listing_args = dict(
        max_accepting_price=str(max_accepting_price), album_id=album_id
    )
    request = request_factory.post(
        f"/users/{user_id}/buyer_account/{buyer_id}/listings",
        data=new_listing_args,
        content_type="application/json",
    )
    response = single_user_single_buyer_account_any_listing(request, user_id, buyer_id)
    assert isinstance(response, JsonResponse)
    ...
    assert "to_buy_listing_id" in json_content and isinstance(
        json_content["to_buy_listing_id"], int
    )
    assert (
        "max_accepting_price" in json_content
        and float(json_content["max_accepting_price"]) == max_accepting_price
    )
    assert (
        "date_posted" in json_content
        and json_content["date_posted"] == date.today().isoformat()
    )
...
def test_single_user_single_buyer_account_any_listing_POST_error_missing_required_properties():
    ...
    assert (
        "message" in json_content
        and json_content["message"]
        == "json object missing required property: 'max_accepting_price'"
    )
...
def test_single_user_single_buyer_account_any_listing_POST_error_unexpected_properties():
    ...
    max_accepting_price = round(1.0 + random.randint(0, 400) / 100, 2)
    new_listing_args = dict(
        max_accepting_price=str(max_accepting_price), album_id=album_id, foo="bar"
    )
    request = request_factory.post(
        f"/users/{user_id}/buyer_account/{buyer_id}/listings",
        data=new_listing_args,
...
def test_single_user_single_buyer_account_any_listing_POST_error_max_accepting_price():
    ...
    assert (
        "message" in json_content
        and json_content["message"]
        == "error in input, property 'max_accepting_price': "
...
def test_single_user_single_buyer_account_single_listing_GET():
    ...
    user_id, buyer_id = user.user_id, user.buyer_id
    listing_ids = [
        listing.to_buy_listing_id
        for listing in ToBuyListing.objects.filter(buyer_id=buyer_id)
    ]
    listing_id = random.choice(listing_ids)
    request = request_factory.get(
        f"/users/{user_id}/buyer_account/{buyer_id}/listings/{listing_id}"
    )
    response = single_user_single_buyer_account_single_listing(
        request, user_id, buyer_id, listing_id
    )
    assert isinstance(response, JsonResponse)
    assert response.status_code == 200
    json_content = json.loads(response.content)
    assert (
        "to_buy_listing_id" in json_content
        and json_content["to_buy_listing_id"] == listing_id
    )
    assert (
        "max_accepting_price" in json_content
        and isinstance(json_content["max_accepting_price"], str)
        and matches_2plc_decimal_format(json_content["max_accepting_price"])
    )
    assert (
        "date_posted" in json_content
        and isinstance(json_content["date_posted"], str)
        and matches_date_isoformat(json_content["date_posted"])
    )
